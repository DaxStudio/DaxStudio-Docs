...
:::info[Note that the time that is allocated to the Formula Engine (denoted by the yellow portions of the Timeline) are simply those times during the query when no Storage Engine events were taking place. We do not have separate events for the formula engine activity.]
:::
...
:::tip[This feature is not visible by default you need to turn it on in the Options - Preview settings]
:::
